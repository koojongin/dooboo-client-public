  ...
  return (
    <div>
      <div className="flex items-center">
        <div className="flex flex-wrap items-center py-[4px] gap-[4px]">
          {menus.map((menu) => {
            return (
              <div
                key={`trade_category_menu_${menu.name}`}
                className="ff-ba ff-skew min-h-[30px] px-[10px] flex items-center justify-center bg-white cursor-pointer border border-gray-600 shadow-md shadow-white/40 rounded"
                onClick={() => {
                  goToRoute(menu.url)
                }}
              >
